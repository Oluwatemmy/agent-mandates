...
RFC_EXAMPLE_CANONICAL = (
    "{"
    + quoted(BACKSLASH + "r") + ":" + quoted("Carriage Return") + ","
    + quoted("1") + ":" + quoted("One") + ","
    + quoted(C1_CONTROL) + ":" + quoted("Control") + ","
    + quoted(O_DIAERESIS) + ":" + quoted("Latin Small Letter O With Diaeresis") + ","
    + quoted(EURO) + ":" + quoted("Euro Sign") + ","
    + quoted(GRINNING_FACE) + ":" + quoted("Emoji: Grinning Face") + ","
    + quoted(DALET_WITH_DAGESH) + ":" + quoted("Hebrew Letter Dalet With Dagesh")
    + "}"
).encode("utf-8")
...
def test_keys_sort_by_utf16_code_unit_not_code_point():
    ...
    serialized = canonical_json_bytes(
        {DALET_WITH_DAGESH: "hebrew", GRINNING_FACE: "emoji"}
    ).decode("utf-8")
...
@pytest.mark.parametrize(
    "character", [chr(0x7F), C1_CONTROL, O_DIAERESIS, EURO, GRINNING_FACE]
)
